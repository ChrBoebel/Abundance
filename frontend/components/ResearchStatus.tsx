  ...
  return (
    <div className="message-bubble flex justify-start mb-4">
      <div className="rounded-2xl px-4 py-3 max-w-5xl mr-4" style={{ background: 'hsl(var(--card))', border: '2px solid hsl(var(--border))' }}>
        ...
        {currentActivity && (
          <div className="text-xs mb-2" style={{ color: 'hsl(var(--foreground) / 0.6)' }}>
            ↳ Aktuell: {currentActivity}
          </div>
        )}

        <div className="text-sm font-semibold mb-3 flex items-center gap-2" style={{ color: 'hsl(var(--primary))' }}>
                ...
                {phases.map((phase) => {
                  ...
                  return (
                    <div key={phase.id}>
                      ...
                      {phase.status === 'running' && currentActivity && (
                        <div className="ml-10 text-xs mt-0.5" style={{ color: 'hsl(var(--foreground) / 0.6)' }}>
                          └ {currentActivity}
                        </div>
                      )}
                    </div>
