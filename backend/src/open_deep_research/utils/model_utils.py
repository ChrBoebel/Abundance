...
def prepare_model_config(config: dict, reasoning_config: Optional[dict] = None) -> dict:
    ...
    Args:
        config: Base model configuration dictionary (with keys: model, max_tokens, api_key)
        reasoning_config: Optional reasoning configuration to add

    Returns:
        Prepared configuration dictionary for .with_config() (RunnableConfig format)
    """
    ...
    # For configurable_fields(), wrap in "configurable" key
    # This is the correct format for RunnableConfig
...
def build_reasoning_config(model_name: str, enable_reasoning: bool, reasoning_effort: str, reasoning_max_tokens: int, exclude_reasoning: bool) -> dict:
    ...
    Args:
        model_name: The model identifier (e.g., "openrouter:google/gemini-2.5-flash")
...
def init_chat_model_wrapper(configurable_fields=None, **kwargs):
    ...
    if configurable_fields:
        ...
        api_key = os.getenv("OPENROUTER_API_KEY") or os.getenv("OPENAI_API_KEY") or "placeholder"
